...
export async function getListedIssues(): Promise<Issue[]> {
  ...
  for (let col of cols) {
    ...
    const numbers = Array.from(
      { length: sheet.cellStats.nonEmpty + 1 },
      (_, i) => sheet.cellStats.nonEmpty - i
    );

    for await (let i of numbers) {
      if (i == 0) {
        // Header row
        let topic: string | undefined = sheet
          .getCellByA1(`${col}${i + 1}`)
          .value?.toString();
        if (topic === undefined) break; // No topic
        topic = topic.trim().replace('(', ' ').split(' ')[0];
        issues.push({ name: topic, subissues: subissues });
      } else {
        // Subissue row
        let notes: string | undefined = sheet
          .getCellByA1(`${col}${i + 1}`)
          .value?.toString();
